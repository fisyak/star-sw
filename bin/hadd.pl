...
foreach my $file (@Files) { print "file = $file\n" if ($ARG{debug});
  ...
  next if ($f =~ /.event.root/);		       
  next if ($f =~ /.MuDst.root/);		       
  next if ($f =~ /.picoDst.root/);		       
  $f =~ s/\.gmt//g;
  $f =~ s/\.root//g;
  $f =~ s/adc_//g;
  $f =~ s/st_//;
...
<job maxFilesPerProcess="1" filesPerHour="10" simulateSubmission="true" fileListSyntax="paths" copyInputLocally="false" >
