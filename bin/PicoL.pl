...
#  $PICOPATH = "/gpfs01/star/data*"; print "PICOPATH = $PICOPATH \n" if ($debug);
  ...
  foreach my $key (sort keys %$def ) {
    ...
    if ($PICOPATH !~ /reco/ and $pwd !~ /TFG24/ and $pwd !~ /2026.dev/) {$glob = "/reco/";}
...
#  my $glb = $run . "/*/*" . $DST . ".root"; # per day
  ...
  for (my $i = 0; $i < $NF; $i += $step) {
    ...
    for (my $k = 0; $k < $step; $k++) {
      my $j = $k + $i;
      if ($k < $NF && $files[$j]) {
	my ($dev, $ino, $mode, $nlink, $uid, $gid, $rdev, $size, $atime, $mtime, $ctime, $blksize, $blocks ) = stat  $files[$j] ;
	my $dt = $now - $ctime;
	if ($dt > 600) {# file is old enough 
	  push @list, $files[$j];
