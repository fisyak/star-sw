...
Int_t StdEdxY2Maker::Make(){ 
  ...
  static  StThreeVectorD xyz;
  static  StThreeVectorD dirG;
  static  Double_t  dx, AdcI, dxC;
#ifdef __dZdY_dXdY__
  ...
  for (UInt_t i=0; i < nTracks; i++) { 
	...
	xyz = StThreeVectorD(tpcHit->position().x(),tpcHit->position().y(),tpcHit->position().z());
	...
	transform(xyz,localSect,sector,row);
	transform(localSect,Pad);
	Double_t slength = tpcHit->lengthInTpc();
	dirG = helixI.momentumAt(slength,bField); // p of global track at hit
	...
	CdEdx[NdEdx].LengthInTpc = tpcHit->lengthInTpc();
	dxC = dx;
#if 1
	...
	if (! TMath::Finite(tpcHit->charge())) {
	  CdEdx[NdEdx].F.mdE.fdE = 1.0;
	} else  {
	  CdEdx[NdEdx].F.mdE.fdE = tpcHit->charge();
	}
  	CdEdx[NdEdx].F.mdx   = dxC;
	CdEdx[NdEdx].xyz[0] = localSect.position().x();
	CdEdx[NdEdx].xyz[1] = localSect.position().y();
	CdEdx[NdEdx].xyz[2] = localSect.position().z();
	...
	CdEdx[NdEdx].ZdriftDistance = localSect.position().z();
	...
	tpcHit->setCharge(CdEdx[NdEdx].F.mdE.fdE);
	if (NdEdx < kNdEdxMax) {
	  TrackLength         += CdEdx[NdEdx].F.mdx;
	  NdEdx++; 
	  NoOfTpcHitsUsed++; 	
	}
	...
	Int_t k;
	for (k = 0; k < N70; k++) {
	  I70 += dEdxS[k].F.mdE.fdEdx;
	  D70 += dEdxS[k].F.mdE.fdEdx*dEdxS[k].F.mdE.fdEdx;
	  TrackLength70 += dEdxS[k].F.mdx;
	  if (dEdxS[k].F.mdx > 0) {
	    SumdEdX += dEdxS[k].F.mdE.fdEdx;
...
void StdEdxY2Maker::SortdEdx() {
  ...
  for (i = 0; i < NdEdx; i++) d[i] = CdEdx[i].F.mdE.fdEdx;
...
__BOOK__VARS__PadTmbk(SIGN,NEGPOS)			\
  ...
  if (PiD.dEdxStatus(kMethod) && PiD.dEdxStatus(kMethod)->TrackLength() > 20) { 
	...
	Double_t Vars[4] = {
	  FdEdx[k].C[StTpcdEdxCorrection::kTpcSecRowB].mdE.fdEdxN, 
	...
	if (tpcGas) {
	  ...
	  if (p > 0) {
	    ...
	    if (temp > 0 &&PressureT[sCharge] ) {
	      ...
	      PressureT[sCharge]->Fill(rowS, pressT, FdEdx[k].F.mdE.fdEdxN);
	    }
	  }
	}
	Double_t vars[2] = {tpcTime,FdEdx[k].C[ StTpcdEdxCorrection::ktpcTime].mdE.fdEdxN};
	if (Time)    Time->Fill(vars);
	//	if (TimeP)  {vars[1] = FdEdx[k].C[StTpcdEdxCorrection::ktpcTime].mdE.fdEdxN; TimeP->Fill(vars);}
...
void StdEdxY2Maker::PrintdEdx(Int_t iop) {
  ...
  for (Int_t i=0; i< NdEdx; i++) {
    dEdx = 0;
    if (iop == 0)      {pdEdx = &CdEdx[i]; dEdx = CdEdx[i].F.mdE.fdEdx;}
    else if (iop == 1) {pdEdx = &FdEdx[i]; dEdx = FdEdx[i].F.mdE.fdEdx;}
    else if (iop == 2) {pdEdx = &dEdxS[i]; dEdx = dEdxS[i].F.mdE.fdEdx;}
    else if (iop >= 3) {pdEdx = &FdEdx[i]; dEdx = FdEdx[i].C[StTpcdEdxCorrection::kUncorrected+iop-3].mdE.fdEdx;}
    I = (i*I + 1e6*pdEdx->F.mdE.fdEdx)/(i+1);
    ...
    cout << Form("%s %2i  S/R %2i/%2i dEdx(keV/cm) %8.2f dx %5.2f dxC %5.2f x[%8.2f,%8.2f,%8.2f] Qcm %7.2f AvC %7.3f", 
		 Names[iop],i,pdEdx->sector,pdEdx->row,dEdx, pdEdx->F.mdx ,pdEdx->dxC, pdEdx->xyz[0], pdEdx->xyz[1], 
...
Double_t StdEdxY2Maker::LikeliHood(Double_t Xlog10bg, Int_t NdEdx, dEdxY2_t *dEdx, Double_t chargeSq) {
  ...
  Double_t f = 0;
  for (Int_t i=0;i<NdEdx; i++) {
    Double_t Ylog2dx = TMath::Log2(dEdx[i].F.mdx);
    //    Double_t Ylog2dx = 1;
    Double_t sigmaC = 0;
    ...
    Double_t xi = (dEdx[i].F.mdE.fdEdxL - GeV2keV - zMostProb)/sigma;
...
void StdEdxY2Maker::DoFitZ(Double_t &chisq, Double_t &fitZ, Double_t &fitdZ){
  Double_t avz = 0;
  for (Int_t i=0;i<NdEdx;i++) avz += FdEdx[i].F.mdE.fdEdxL;
  if (NdEdx>5) {
    avz /= NdEdx;
    Double_t arglist[10];
    Int_t ierflg = 0;
...
void StdEdxY2Maker::fcnN(Int_t &npar, Double_t *gin, Double_t &f, Double_t *par, Int_t iflag) {
  ...
  if (_debug > 0 && iflag == 1) {
    ...
    if (ix <= 0) {
      if (!c1) c1 = new TCanvas("fcn","fcn",1000,1000);
      else     c1->Clear();
      //    c1->Divide(1,3);
      if (ix < 0) c1->Divide(1,2);
      else        c1->Divide(2,2);
    }
  }
  ...
  f = 0;
  gin[0] = 0.;
  Double_t dNdx0 = par[0]; // Mu
  //  Double_t sigma = par[1]; // extra sigma
  for (Int_t i = 0; i < NdEdx; i++) {
    Double_t length = FdEdx[i].LengthInTpc;
    Double_t scale = (1 + par[1]*length);
    if (scale < 0.5 || scale > 1.5) {
      f += 100;
      FdEdx[i].Prob = 0;
      continue;
    }
    Double_t dNdx = dNdx0*scale;
  ...
  if (_debug > 0) {
	...
	//	IA[i] = I[i];
      ...
      if (ix < 0) c1->cd(2)->SetLogy(1);
      else        c1->cd(2+ix)->SetLogy(1);
...
void StdEdxY2Maker::DoFitN(Double_t &chisq, Double_t &fitZ, Double_t &fitdZ){
  Double_t dNdx = 1e9*TMath::Exp(fitZ)/92.24/1.4; // 0.080; // 80 eV per primary interaction
  ...
  static Double_t slopeMin = -1e-3, slopeMax = 5e-2;
  m_Minuit->DefineParameter(1, "slope", 0.00, 0.01, slopeMin, slopeMax);
  ...
  m_Minuit->GetParameter(0, fitZ, fitdZ);
  if (Debug()) {
    cout << "StdEdxY2Maker::DoFitN: dNdx = " << dNdx << "\tfit = " << fitZ << "\tratio = " << dNdx/fitZ << endl;
  }
#if 0
  // 2.
  arglist[0] = 1.0;
