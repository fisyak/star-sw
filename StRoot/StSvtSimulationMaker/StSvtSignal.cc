...
double StSvtSignal::analConvInt(double tim, double sigmat, double tc)
{
 ...
 double sigmat2,sigmat3,sigmat4;
  ...
  //bc5 = b/::pow(c,5);   bc4 = b/::pow(c,4);    bc3 = b/(2*::pow(c,3));   bc2 = b/(6*c*c);    ac1 = a/(24*c);
   ...
   Double_t ErraXexpa = Erra * expa;
   Double_t chi2 = ta2/sigmat2;
   gaus = 0;
   Double_t gausXexpa = 0;
   if (chi2 < 100) {
     gaus = (sigmat/::sqrt(2*M_PI))*exp(-0.5*chi2);
   }
   gausXexpa = gaus*expa;
 ...
 /* At = bc5*expb*Errb + ((-bc5 + bc4*ta - bc3*(ta2 + sigmat2) + bc2*(ta3 + 3.0*ta*sigmat2) 
  ...
  At = bc5*expb*Errb;
  At += (-bc5 + bc4*ta - bc3*(ta2 +     sigmat2) + bc2*(ta3 + 3.0*ta*sigmat2) - ac1*(ta4 + 6.0*ta2*sigmat2 + 3*sigmat4))*ErraXexpa;
  At += ( bc4 - bc3*ta + bc2*(ta2 + 2.0*sigmat2)	- ac1*(ta3 + 5.0*ta*sigmat2)                                    )*gausXexpa;            
...
double StSvtSignal::freq(double num)
{
 ...
 if(num > 0.0)
  frq = 0.5 + 0.5*TMath::Erfc( num/M_SQRT2);
 else
  frq =  0.5* TMath::Erfc(num/M_SQRT2);
