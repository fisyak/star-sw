...
public:
  ...
  void  operator()(const        StGlobalCoordinate& a, StTpcLocalCoordinate& b,Int_t sector = 0, Int_t row = 0)
  {StTpcDb::instance()->Tpc2GlobalMatrix().MasterToLocal(a.position().xyz(),b.position().xyz()); 
    if (sector >  0 && sector <= 24) b.setSector(sector);
    else                             b.setSectorFromCoordinates(); 
    b.setRow(row);}
