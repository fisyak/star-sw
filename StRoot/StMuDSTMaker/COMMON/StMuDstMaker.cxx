...
StMuDstMaker::StMuDstMaker(const char* name) : StIOInterFace(name),
  ...
  mFmsCollection(0),mRHICfCollection(0),mFcsCollection(0),mFttCollection(0),mFstCollection(0),mFwdTrackCollection(0), mPmdCollectionArray(0), mPmdCollection(0)
...
void StMuDstMaker::setBranchAddresses(TChain* chain) {
  ...
  if (!mFttCollection) {
    mFttCollection=new StMuFttCollection();
    connectFttCollection();
    mStMuDst->set(this);
  }

  if (!mFstCollection) {
    mFstCollection=new StMuFstCollection();
    connectFstCollection();
    mStMuDst->set(this);
  }

  if (!mFwdTrackCollection) {
    mFwdTrackCollection=new StMuFwdTrackCollection();
    connectFwdTrackCollection();
    mStMuDst->set(this);
  }
...
//-----------------------------------------------------------------------
//-----------------------------------------------------------------------
void StMuDstMaker::fillFtt(StEvent* ev) {
  DEBUGMESSAGE2("");
  StFttCollection* fttcol=(StFttCollection*)ev->fttCollection();
  if (!fttcol)  return; //throw StMuExceptionNullPointer("no StFttCollection",__PRETTYF__);
  StTimer timer;
  timer.start();
  ...
  mFttUtil->fillMuFtt(mFttCollection,fttcol);
...
//-----------------------------------------------------------------------
//-----------------------------------------------------------------------
void StMuDstMaker::fillFwdTrack(StEvent* ev) {
  DEBUGMESSAGE2("");
  LOG_INFO << "StMuDstMaker::fillFwdTrack(StEvent* ev)" << endm;
  StFwdTrackCollection* fwdcol=(StFwdTrackCollection*)ev->fwdTrackCollection();
  if (!fwdcol)  return; //throw StMuExceptionNullPointer("no StFstHitCollection",__PRETTYF__);
  StTimer timer;
  timer.start();

  LOG_INFO << "StMuDSTMaker filling StMuFwdTrackCollection from StEvent" << endm;

  if (!mFwdTrackCollection) {
    mFwdTrackCollection=new StMuFwdTrackCollection();
    LOG_INFO << "Connecting StMuFwdTrackCollection" << endm;
    connectFwdTrackCollection();
    mStMuDst->set(this);
  }

  mFwdTrackUtil->fillMuFwdTrack(mFwdTrackCollection,fwdcol,mFcsUtil);
...
void StMuDstMaker::fillEzt(StEvent* ev) {
  ...
  char *eztArrayStatus=&mStatusArrays[__NARRAYS__+
                      ...
                      __NSTRANGEARRAYS__+
#endif
                      __NMCARRAYS__+__NEMCARRAYS__+__NPMDARRAYS__+__NFMSARRAYS__+__NRHICFARRAYS__+__NFCSARRAYS__+__NFTTARRAYS__+__NFSTARRAYS__+__NFWDTRACKARRAYS__+
...
//-----------------------------------------------------------------------
void StMuDstMaker::connectFttCollection() {
  LOG_INFO << "Setting Ftt arrays" << endm;
  mFttCollection->setFttHitArray(mFttArrays[muFttRawHit]);
  mFttCollection->setFttClusterArray(mFttArrays[muFttCluster]);
  mFttCollection->setFttPointArray(mFttArrays[muFttPoint]);
}
//-----------------------------------------------------------------------
...
//-----------------------------------------------------------------------
void StMuDstMaker::connectFwdTrackCollection() {
  LOG_INFO << "Setting FwdTrack arrays" << endm;
  mFwdTrackCollection->setFwdTrackArray(mFwdTrackArrays[muFwdTrack]);
}
//-----------------------------------------------------------------------
