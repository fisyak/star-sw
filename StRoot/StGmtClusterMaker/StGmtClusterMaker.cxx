...
#else
  UInt_t nClusXY[2] = {nClusX, nClusY};
  TF1 *fitXY[2] = {fitX, fitY};
  UInt_t *idXY[2] = {idx, idy};
  static const Char_t *XY[2] = {"X", "Y"};
  for (Int_t ixy = 0; ixy < 2; ixy++) {
    for(UInt_t i=0; i < nClusXY[ixy]; i++) {
      UInt_t j = idXY[ixy][i];
      StGmtHit* hit = new StGmtHit(hits.getHitVec().size()+1,
				   module, ixy,
				   fitXY[ixy]->GetParameter(3*j+1),             // adcLog
				   fitXY[ixy]->GetParError(3*j+1),              // error(adc)
				   fitXY[ixy]->GetParameter(3*j+2),             // mean
				   fitXY[ixy]->GetParError(3*j+2),              // error(mean)
				   fitXY[ixy]->GetParameter(3*j+3),             // sigma
				   fitXY[ixy]->GetParError(3*j+3));             // error(sigma)
