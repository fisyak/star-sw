...
**************************/
L2eventStream2012  globL2eventStream2012;
/*************************/
...
L2btowCalAlgo12::L2btowCalAlgo12(const char* name, const char *uid, L2EmcDb2012* db, L2EmcGeom2012 *geoX, char* outDir, int resOff)  :  L2VirtualAlgo2012( name, uid,  db,  outDir, true, false, resOff) { 
  ...
  */
  ...
  for(k=0;k<L2eventStream2012::mxToken;k++){
    L2BtowCalibData12 & btowCalibData=globL2eventStream2012.btow[k];  
...
/* ========================================
  ======================================== */
int 
L2btowCalAlgo12::initRunUser( int runNo, int *rc_ints, float *rc_floats) {
...
L2btowCalAlgo12::calibrateBtow(int token, int bemcIn, unsigned short *rawAdc){
  ...
  //LOG("JEFF", "Token=%d",token);
  token&=L2eventStream2012::tokenMask; // only protect against bad token, Gerard's trick
  //LOG("JEFF", "Token=%d",token);

  //...... token is valid no w ........
  L2BtowCalibData12 & btowCalibData=globL2eventStream2012.btow[token];  
  ...
  if(bemcIn && par_gainType>kGainZero) { // EVEVEVEVEVE
    ...
    for(rdo=0; rdo<BtowGeom::mxRdo; rdo++){
      ...
      //LOG("JEFF", "rawAdc[%d] = %d  thr=%d  ped=%d",rdo,rawAdc[rdo],thr[rdo],ped[rdo],BtowGeom::mxRdo);

      if(nTower>=L2BtowCalibData12::mxListSize) break; // overflow protection
    ...
    if(nTower>=L2BtowCalibData12::mxListSize) mhN->fill(5); // was overflow
  
  } // EVEVEVEVEVE

  LOG(DBG, "hits = %d   hot=%d",nTower,nHotTower);
...
void
L2btowCalAlgo12::clear(int token){
  token&=L2eventStream2012::tokenMask; // only protect against bad token, Gerard's trick
  //...... token is valid no w ........
  L2BtowCalibData12 & btowCalibData=globL2eventStream2012.btow[token];
  btowCalibData.hitSize=0;
...
/* ========================================
  ======================================== */
void
L2btowCalAlgo12::finishRunUser() {  /* called once at the end of the run */
...
//=======================================
void 
L2btowCalAlgo12::createHisto() {
  memset(hA,0,mxHA*sizeof(L2Histo*));
  //token related spectra
  hA[1]=new  L2Histo(1,"L2-btow-calib: seen tokens;  x:  token value; y: events ",L2eventStream2012::mxToken);
...
/* ========================================
  ======================================== */
void 
L2btowCalAlgo12::print0(){ // full raw input  ADC array
  // empty
 }
