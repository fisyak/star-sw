...
int StFmsBitCheckMaker::Make(){
  ...
  unsigned long long  bunchid7  = td->bunchId7Bit();
  unsigned long long  bunchid48 = td->bunchId48Bit();
  if(bx%120 != bunchid48)
    printf("BC48 check :  (BX from BCdata)120=%d is not consistent with  bunchId48Bit=%d\n",(int)bx%120,(int)bunchid48);
  ...
  if(dd!=0)
    printf("BC7: bc7=%3d bc48=%3d bc48corr=%3d D=%3d D0=%3d DD=%3d BC48=%llx\n",(int)bunchid7,(int)bunchid48,bc48c,dbc,dbc0,dd,bx);
  //if(td->bcData(1)>0)
  if((td->bcData(12)&0x1)==1 && bunchid7!=1) {
    //if(bunchid7<4 || bunchid7>117)
    printf("Revtick : bc7=%3d %3d RT1=%x %x %x %x %x %x RT2=%1d BC48=%dlx\n",
	   (int)bunchid7,td->bcData(2)&0x7f,
	   td->bcData(1),
	   td->bcData(0),
	   td->bcData(4),
	   td->bcData(5),
