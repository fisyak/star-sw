...
StEventMaker::makeEvent()
{
    ...
    if (daqReaderSet) { 
      StTrigSummary* trigSummary =
		...
		ULong64_t mask = 0;
		mask  = (UInt_t) trigSummary->L1summary[1];
		mask <<= 32;
		mask += (UInt_t) trigSummary->L1summary[0];
		trigId[0]->setMask(mask);

		mask  = (UInt_t) trigSummary->L2summary[1];
		mask <<= 32;
		mask += (UInt_t) trigSummary->L2summary[0];
		trigId[1]->setMask(mask);

		mask  = (UInt_t) trigSummary->L3summary[1];
		mask <<= 32;
		mask += (UInt_t) trigSummary->L3summary[0];
	    ...
	    for(UInt_t trglevel=0 ; trglevel < 3 ; trglevel++){
		StTriggerId* whichTrig =  trigId[trglevel];

		// Loop over the triggers within this level
		for (UInt_t iTrg = 0; iTrg < dbTriggerId->getIDNumRows() ; iTrg++){
		    // Shift the mask by daqTrigId bits to examine that bit
		    if ( whichTrig->mask() &  ((ULong64_t)1 << (dbTriggerId->getDaqTrgId(iTrg)) )  ) {
		...
		for (UInt_t iTrg = 0; iTrg < dbTriggerId->getAdditionalTriggerIDNumRows() ; iTrg++){
		    // Shift the mask by daqTrigId bits to examine that bit
		    // Tweak for warnings; saved the run/event number as an UInt_t, but for some reason StEvent is int
		...
		for (UInt_t irow=0; irow<dbTriggerId->getTrigL3ExpandedNumRows(); ++irow) {
		    UInt_t oldtid = dbTriggerId->getTrigL3ExpandedL3TrgId(irow);
		    UInt_t newtid = dbTriggerId->getTrigL3ExpandedL3ExpandedTrgId(irow);
...
// We need to make a copy of the nominal to avoid problems in delete??
		...
		if (IAttr("laserIT")) {
	      ...
	      St_trgOfflineFilter* flaggedTrgs =
              ...
              if (tr) {
                vector<UInt_t> idVec = tr->triggerIds();
...
StEventMaker::printEventInfo()
{
 ...
 StEventSummary *evtsum = mCurrentEvent->summary();
    ...
    if (mCurrentEvent->trackNodes().size()) {
	  ...
	  printTrackInfo((StTrack *)mCurrentEvent->trackNodes()[0]->track(i));
    }
