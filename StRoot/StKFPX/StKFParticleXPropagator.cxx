...
StKFParticleXPropagator::StKFParticleXPropagator() {//: KFParticleXPropagator() {
  ...
  MyMat_t myMat[] = { 
    ...
    {"Vacuum"            ,"mat",  0,   0.000,   3.900,   1.000,   0.000, 0.000e+00, 1.000e+11, -76.200,  76.200, 0},//PIPE_1/PIPC_1        0.29%
    {"BERILLIUM"         ,"mat",  1,   3.900,   4.000,   9.010,   4.000,     1.848, 3.446e+01, -76.200,  76.200, 0},//PIPE_1/PIPC_1
    {"PIPE"              ,"mat",  3,   3.875,   4.000,  26.980,  13.000,     2.700, 8.875e+00,  76.200, 383.000, 0},//PIPE_1/PIPO_1/PVAO_1 1.40%
    ...
    {"ALUMINIUM_TIFC"    ,"mat",  7,  46.600,  51.700,  21.181,  10.311,     0.089, 3.220e+02,-230.876, 230.484, 0},///TIFC_1              1.58%
    {"TPCE_SENSITIVE_GAS","gas",  8,  51.700, 200.000,  38.691,  17.436, 1.541e-03, 1.302e+04,-224.186, 223.794, 0},///TOFC_1              1.14%
    {"ALUMINIUM_TOFC"    ,"mat",  9, 200.000, 207.731,  26.827,  12.886,     0.324, 7.480e+01,-224.186, 223.794, 0} ///TOFC_1             10.333%
  };
...
Bool_t StKFParticleXPropagator::Propagete2Radius(const KFParticle &p, Float_t Radius) {
  ...
  if (CurrentR < Radius) {
	...
	CurrentZ = fgParticle.GetZ();
	cout << "Target R " << R << "\tCurrent R = " << CurrentR << "\tCurrent Z = " << CurrentZ << endl;
	KFParticle temp = fgParticle;
	temp.TransportToDS(-dS, dsdr);
	cout << "Forward  "; fgParticle.Print();
	cout << "Backward "; temp.Print();
      } else {
	Float_t xy[2] = {0,0};
...
void StKFParticleXPropagator::Test(const Option_t *opt) {
  ...
  for (Int_t t = 0; t < 2; t++) {
    ...
    track.SetID(t+1);
    KFParticle particle1(track, pdg); cout << "1st point particle1" << endl; particle1.Print();
    particle.TransportToPoint(&particle1.Parameter(0)); cout << "After Transport dca to 1st\n"; particle.Print();
#if 1
    KFParticle particle2 = particle1;
    cout << "t = " << t << "--------------------------------------------------------------------------------" << endl;
    cout << "Transport particle from 1st to 0" << endl;
    R = 0;
