...
//________________________________________________________________________________
void St_TpcAvgPowerSupplyC::PrintC(Option_t *opt) const {
  ...
  if (! TString(opt).Contains("V",TString::kIgnoreCase)) {
    for (Int_t sec = 1; sec <= 24; sec++) {
      cout << "Current " << sec;
...
Bool_t        St_beamInfoC::IsFixedTarget() {
  ...
  else if (year == 2026 && (/* 2026:      9p8GeV_fixedTarget_2026       */ ( run >= 27021017 && run <  27032006)) ) return kTRUE;
  return kFALSE;
#if 0
