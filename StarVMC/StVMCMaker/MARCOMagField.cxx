...
MARCOMagField::MARCOMagField() : TVirtualMagField("MARCOMagField"), fBr(0), fBz(0) { 
  ...
  const Char_t *rootf = "MARCO_v.6.4.1.1.3_1.7T_Magnetic_Field_Map_2022_11_14_rad_coords_cm_T.root";
...
//________________________________________________________________________________
MARCOMagField *MARCOMagField::Instance() {
  if (! fgInstance) fgInstance = new MARCOMagField();
  return fgInstance;
}
//________________________________________________________________________________
void MARCOMagField::Field    ( const Double_t *x, Double_t *B ) {
  ...
  B[0] = B[1] = B[2] = 0;
  if (r > fBr->GetXaxis()->GetXmax()) return;
  if (z < fBr->GetYaxis()->GetXmin() ||
      z > fBr->GetYaxis()->GetXmax()) return;
