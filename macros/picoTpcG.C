   ...
   foreach d (`ls -1d 14[5-6]/2*`)
     cd ${d}
if (!  -r picoTpcG.root) then 
     ln -s ../../.sl* .
     root.exe -q -b 'lMuDst.C(-1,"*picoDst.root","RpicoDst,tpcDb,detDb,mysql,magF,nodefault,CorrY,quiet","picoTpcG.root")'  picoTpcG.C+ >& picoTpcG.log &
     cd -
endif
   ...
   root.exe lMuDst.C picoTpcG.root
   .L picoTpcG.C+
   Draw();
   root.exe picoTpcG.root picoTpcGPrint.C
...
//________________________________________________________________________________
//________________________________________________________________________________
Bool_t Accept(const StPicoTrack *gTrack = 0) {
  ...
  if (  gTrack->gPt() < 0.5) return kFALSE;
...
Int_t SectorNumber(Float_t x, Float_t y, Float_t z) {
  TVector3 glob(x,y,z);
  TVector3 tpc;
  StTpcDb::instance()->Tpc2GlobalMatrix().MasterToLocal(&glob[0],&tpc[0]);
...
//________________________________________________________________________________
Int_t SectorNumber(Double_t xyz[3]) {
  return SectorNumber(xyz[0], xyz[1], xyz[2]);
}
//________________________________________________________________________________
Int_t SectorNumber(const TVector3 &position) {
...
//________________________________________________________________________________
void Process1Event(StPicoDst* picoDst = 0, Long64_t ev = 0) {
  ...
  if (! picoDst) return;
  Int_t date = picoDstMaker->GetDateTime().Convert() - timeOffSet;
  if (ev%1000 == 0) cout << "Read event\t" << ev << endl;
  StPicoEvent* picoEvent = picoDst->event(); // get a pointer to the class holding event-wise information
  Double_t Bz = picoEvent->bField();
  Double_t vpdZ = picoEvent->vzVpd();
  ...
  if (Debug()) {cout << " #" << ev;}
  Int_t NoGlobalTracks = picoDst->numberOfTracks( );  if (Debug()) {cout << "\tGlobalTracks " << NoGlobalTracks;}
  // Count no. of good primary tracks
  Int_t noGoodPrimTracks = 0;
  for (Int_t k = 0; k < NoGlobalTracks; k++) {
    StPicoTrack *gTrack = picoDst->track(k);
    if (!gTrack) continue;
    if (!gTrack->isPrimary()) continue;
    StPicoTrack *pTrack = gTrack;
    StPicoTrackCovMatrix *gCov = picoDst->trackCovMatrix(k);
    if (! gCov) continue;
    noGoodPrimTracks++;
  }
  ...
  if (vzVpd > -998.9 && TMath::Abs(zTpc - vzVpd) > 10.0) return;

  TVector3 VGlob = pRcVx;                                       PrPP(VGlob);
  const TGeoHMatrix &Tpc2Global = StTpcDb::instance()->Tpc2GlobalMatrix();
  TVector3 VTpc;                                
  Tpc2Global.MasterToLocal(&VGlob[0], &VTpc[0]);            PrPP(VTpc);
  for (Int_t kg = 0; kg < NoGlobalTracks; kg++) {
    ...
    StPicoTrack *pTrack = gTrack;
    PrPP3(*pTrack);
    StPicoTrackCovMatrix *gCov = picoDst->trackCovMatrix(kg);
    ...
    thelixK.Dca(&VGlob[0],dca[0],dca[1],ermx,2);
    if (TMath::Abs(dca[0]) > 2 || TMath::Abs(dca[1]) > 5) continue;
    Double_t RI =  60;
    Double_t RO = 180;
    Double_t surfI[6] = {-RI*RI, 0, 0, 0, 1, 1};
    THelixTrack thelixKI = thelixK;
    Double_t xyzI[3];
    Double_t sI = thelixKI.Step(100000, surfI, 6, xyzI);
    Int_t sectorI = SectorNumber(xyzI);
    Double_t surfO[6] = {-RO*RO, 0, 0, 0, 1, 1};
    THelixTrack thelixKO = thelixK;
    Double_t xyzO[3];
    Double_t sO = thelixKO.Step(100000, surfO, 6, xyzO);
    Int_t sectorO = SectorNumber(xyzO);
    if (sectorI != sectorO) continue;
    particle.SetIdParentMcVx(sectorI); PrPP3(particle);
    particle.S() = dcaG.curvature();
    particles[0].push_back(particle);
    if (sectorI <= 12) particles[1].push_back(particle);
  ...
  if (Vertex[1].GetChi2() > 0 && Vertex[2].GetChi2() > 0) {
    TVector3 W(Vertex[1].GetX(), Vertex[1].GetY(), Vertex[1].GetZ());
    TVector3 E(Vertex[2].GetX(), Vertex[2].GetY(), Vertex[2].GetZ());
    TVector3 WTpc, ETpc;
    Tpc2Global.MasterToLocal(&W[0],&WTpc[0]); PrPP(WTpc);
    Tpc2Global.MasterToLocal(&E[0],&ETpc[0]); PrPP(ETpc);
    TVector3 sum = 0.5*(WTpc+ETpc);
    TVector3 dif = 0.5*(WTpc-ETpc);
  ...
  for (UInt_t i = 0; i < npart; i++) {
...
#endif
    Glob2SupS(sector,&VGlob[0], &vxS[0]); PrPP(vxS);
    TVector3 vx(Vertex[0].GetX(), Vertex[0].GetY(), Vertex[0].GetZ()); PrPP(vx);
    TArrayF dsdr(6);
    Float_t xyzV[3] = {Vertex[0].GetX(), Vertex[0].GetY(), Vertex[0].GetZ()}; 
    Float_t ds = particle.GetDStoPoint(xyzV,dsdr.GetArray());    PrPP3(ds);
    ...
    TVector3 x(pars);   PrPP2(x);
    TVector3 p(pars+3); PrPP2(p);
    TVector3 vS, xS, pS; // in sector CS
    //    StTpcDb::instance()->SupS2Tpc(sector).MasterToLocal(&vx[0],  &vS[0]);  PrPP2(vS);
    TVector3 vxD = vx;
    Glob2SupS(sector,&vxD[0], &vS[0]);    PrPP2(vS);
    Glob2SupS(sector,&x[0],  &xS[0]);     PrPP2(xS);
    Glob2SupSVect(sector, &p[0], &pS[0]); PrPP2(pS);
    TVector3 d = xS - vS; if (Debug() > 1) cout << "sector:" << sector << "\td: " << d << endl;
    ...
    TVector3 xTpc, vxTpc;
    Tpc2Global.MasterToLocal(&x[0],&xTpc[0]);
    Tpc2Global.MasterToLocal(&vxD[0],&vxTpc[0]);
    TVector3 dTpc = xTpc - vxTpc;
...
//________________________________________________________________________________
void picoTpcG(Long64_t nEvents = 10000000) {
  StBFChain *chain = (StBFChain *) StMaker::GetTopChain();
  picoDstMaker = (StPicoDstMaker *) chain->Maker("PicoDst");
  if (! picoDstMaker) return;
  ...
  const Char_t *ActiveBranches[] = {"Event",
				    "Track",
				    "EmcTrigger",
				    "MtdTrigger",
				    "BTowHit",
  ...
  for (Int_t i = 0; i < Nb; i++) picoDstMaker->SetStatus(ActiveBranches[i],1); // Set Active braches
  TChain *tree = picoDstMaker->chain();
  if (! tree) return;
  ...
  for (Long64_t ev = 0; ev < nEvents; ev++) {
    iok = chain->MakeEvent();
    if (iok) break;
    StPicoDst* picoDst = picoDstMaker->picoDst();   // get a pointer to the StPicoDst class, the class that points to all the data
    Process1Event(picoDst,ev);
  }
  return;
}
//________________________________________________________________________________
