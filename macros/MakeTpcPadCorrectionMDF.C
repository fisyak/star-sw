...
/*
  root.exe -q -b xyPad3CGF_y3RunXXI05.root 'MakeTpcPadCorrectionMDF.C("mu",7,20,20210129,5)' >& MakeTpcPadCorrectionMDF.log
  foreach b (COL FXT)
     root.exe -q -b xyPad3CGF_y3${b}.root  'MakeTpcPadCorrectionMDF.C("mu",7,20,20210129,5)'| tee ${b}.log; mv TpcPadCorrectionMDF.20210129.000005.C TpcPadCorrectionMDF.${b}.C
  end 
...
dir TpcZCorrectionB.20* | grep GeV | awk '{print "ln -s "$11" "$9}' | sed -e 's/TpcZCorrectionB/TpcPadCorrectionMDF/g'
  foreach f (`ls -1d xyPad3+xyPad3PG4EY*.root`)
     set b = `basename ${f} .root | sed -e 's/xyPad3+xyPad3PG4EY//' -e 's/_y3//'`;
     echo "${b}"
...
void h2mdf(const TH2D *total2D, Int_t max=7, Int_t maxTerm = 20){
  Int_t nVars       = 2;
...
void MakeTpcPadCorrectionMDF(const Char_t  *total = "mu", Int_t max=7, Int_t maxTerm = 20, Int_t date = 20190201, Int_t time = 709){
