...
TF1 *FitGG(TH1 *proj, Option_t *opt="RQ", TH1D *proj20 = 0) {
  ...
  if (proj20) {
  if (! canvas && ! gROOT->IsBatch() ) {
    canvas = new TCanvas("canvas","FitGG");
  }
#if 0
    proj20->Fit("gaus");
    TF1 *gaus = (TF1 *) proj20->GetListOfFunctions()->FindObject("gaus");
