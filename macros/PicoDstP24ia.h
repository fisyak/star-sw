...
// Fri Jul 26 11:08:14 2024 by ROOT version 5.34/39
// from TTree PicoDst/StPicoDst
// found on file: st_physics_20183003_raw_7500001.picoDst.root
//////////////////////////////////////////////////////////

#ifndef PicoDstP24ia_h
#define PicoDstP24ia_h
...
// Fixed size dimensions of array or collections stored in the TTree if any.
  enum {
   ...
   kMaxFmsHit = 1,
   kMaxEmcPidTraits = 32,
   kMaxBTofPidTraits = 144,
   kMaxMtdPidTraits = 1,
   kMaxTrackCovMatrix = 1116,
   kMaxBEmcSmdEHit = 1,
   kMaxBEmcSmdPHit = 1,
   kMaxETofHit = 115,
   kMaxETofPidTraits = 30,
   kMaxMcVertex = 1,
   kMaxMcTrack = 1
  };
class PicoDstP24ia {
public :
   ...
   PicoDstP24ia(TTree *tree=0);
   virtual ~PicoDstP24ia();
...
#ifdef PicoDstP24ia_cxx
PicoDstP24ia::PicoDstP24ia(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("st_physics_20183003_raw_7500001.picoDst.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("st_physics_20183003_raw_7500001.picoDst.root");
      }
...
PicoDstP24ia::~PicoDstP24ia()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t PicoDstP24ia::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t PicoDstP24ia::LoadTree(Long64_t entry)
{
...
void PicoDstP24ia::Init(TTree *tree)
{
...
void PicoDstP24ia::Show(Long64_t entry)
{
...
Int_t PicoDstP24ia::Cut(Long64_t entry)
{
