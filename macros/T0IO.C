...
*/
void DrawT0(TString Opt = "T0(I-O)") {
  struct Par_t {
  ...
  Par_t Pars[][4] = {
    {
      ...
      {  13.99406,   8.714198, 0.02974651, "MuEI", "RF/2019/AuAu200GeV"} // 
    }, {
  ...
  Int_t io1 = 0, io2 = 0;
  Int_t kase = 0;
  if (Opt == "T0(I-O)") kase = 1;
  if (Opt == "T0I")    {io1 = io2 = 1;}
  if (Opt == "T0I&O")  {io1 = 0; io2 = 1;}
  Double_t ymin =  9999;
  Double_t ymax = -9999;
  for (Int_t cf = 0; cf < 2; cf++) {// 0 -> Collider; 1 -> fixedTarget
    if (cf == 0) COLFXT = "COL";
    else         COLFXT = "FXT";
    for (Int_t io = io1; io <= io2; io++) {// 0 -> O, 1 -> I
	...
	for (Int_t ff = 0; ff < 2; ff++) { // 0 ->", "FF, 1 ->", "RF
	  ...
	  for (Int_t i = 0; i < N; i++) {
	    ...
	    if (y[np] + e[np] > ymax) ymax = y[np] + e[np];
	    if (y[np] - e[np] < ymin) ymin = y[np] - e[np];
  ...
  mg->Draw("axp");
  l->Draw();
  if (x2019 > -1) {TLine *line = new TLine(x2019, ymin, x2019, ymax); line->Draw(); } 
  if (x2020 > -1) {TLine *line = new TLine(x2020, ymin, x2020, ymax); line->Draw(); } 
  if (x2021 > -1) {TLine *line = new TLine(x2021, ymin, x2021, ymax); line->Draw(); } 
  if (x2022 > -1) {TLine *line = new TLine(x2022, ymin, x2022, ymax); line->Draw(); } 
  if (x2023 > -1) {TLine *line = new TLine(x2023, ymin, x2023, ymax); line->Draw(); } 
}
