/*
 ...
 MuDst->Draw("1./PrimaryTracks.mBTofPidTraits.mBeta:PrimaryTracks.mP.mag()","PrimaryTracks.mBTofPidTraits.mBeta>0","colz")
 MuDst->Draw("1./PrimaryTracks.mETofPidTraits.mBeta:PrimaryTracks.mP.mag()","PrimaryTracks.mETofPidTraits.mBeta>0","colz")
...
Double_t ToFMass2(StThreeVectorF P, Float_t beta) {
  if (beta <= 0 || beta > 1.0) return -1.;
  Double_t mass2 = P.mag2()*(1./(beta*beta) - 1);
  cout << "P = " << P << "\tbeta = " << beta << " mass2 = " << mass2 << endl;
  return mass2;
}
