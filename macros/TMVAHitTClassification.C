...
///
///     root  'Chain.C+("*.root","HitT")' TMVAHitTClassification.C+\(\"Fisher,Likelihood\"\)
///
#include <cstdlib>
#include <iostream>
#include <map>
...
int TMVAHitTClassification( TString myMethodList = "" )
{
   ...
   //
   //     mylinux~> root -l TMVAHitTClassification.C\(\"myMethod1,myMethod2,myMethod3\"\)
   ...
   TString outfileName("TMVACHitT.root");
...
// dataloader->AddSpectator( "flagL" ,  "flagL"  , "units", 'F' ); NVars++; static const Float_t&     flagL                                    = iter("flagL"); 
   ...
   dataloader->AddVariable ( "padOv" ,  "padOv"  , "units", 'F' ); NVars++; static const Float_t&     padOv                                    = iter("padOv"); 
   dataloader->AddVariable ( "tbkOv" ,  "tbkOv"  , "units", 'F' ); NVars++; static const Float_t&     tbkOv                                    = iter("tbkOv"); 
#endif
  ...
  while (iter.Next() > 0) {
    ...
    if (IdTK == IdTL && QAK > 90 && QAL > 90) { // Signal
      evS++;
