...
TDataSet *CreateTable() { 
  ...
  // SecRow3CGGAuAu_2025;  FitP->Draw("sqrt(sigma**2-0.397108**2):y>>O(72,0.5,72.5)","i&&j&&y>40.5","sameprof")
  row.SecRowSigOW[0] = row.SecRowSigOE[0] =    0.494312;//   +/-   0.0178456  
  row.SecRowSigOW[1] = row.SecRowSigOE[1] = -0.00419568;//   +/-   0.000632127
  row.SecRowSigOW[2] = row.SecRowSigOE[2] = 2.77447e-05;//   +/-   5.47174e-06
  //FitP->Draw("sqrt(sigma**2-0.414444**2):y>>IW(72,0.5,72.5)","i&&j&&y<40.5&&x<12.5","prof")
  row.SecRowSigIW[0] =    0.395532;//   +/-   0.0030723   
  row.SecRowSigIW[1] = -0.00672511;//   +/-   0.000753367 
  row.SecRowSigIW[2] = 0.000366621;//   +/-   4.85163e-05 
  row.SecRowSigIW[3] = -5.0581e-06;//   +/-   8.47865e-07 
  // FitP->Draw("sqrt(sigma**2-0.414444**2):y>>IE(72,0.5,72.5)","i&&j&&y<40.5&&x>12.5","sameprof") 
  row.SecRowSigIE[0] =     0.437163;//   +/-   0.00458552 
  row.SecRowSigIE[1] =   -0.0109511;//   +/-   0.000933245
  row.SecRowSigIE[2] =  0.000511021;//   +/-   5.288e-05  
  row.SecRowSigIE[3] = -6.10741e-06;//   +/-   8.53982e-07
  row.PolyaInner = 1.38;
  row.PolyaOuter = 1.38;
