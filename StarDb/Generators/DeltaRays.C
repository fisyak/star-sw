...
TDataSet *CreateTable() {
  ...
  if ( gClassTable->GetID("TGiant3") >= 0) { // root4star
    ...
    if (! gener) gener =  new 
      StarMCSimplePrimaryGenerator( Npart, iD, pTlow,pThigh,Ylow, Yhigh, Philow, Phihigh, Zlow, Zhigh, "DeltaRays");
    else
      gener->SetGenerator( Npart, iD, pTlow,pThigh,Ylow, Yhigh, Philow, Phihigh, Zlow, Zhigh, "DeltaRays");
