...
#$INSU::COMPDIRAFS="/afs/.rhic/star/replicas/DEV";
$INSU::STARCVMFS="/cvmfs/star.sdcc.bnl.gov/packages";
$INSU::COMPDIRAFS=$INSU::STARAFS."/adev";
$INSU::COMPDIRCVMFS=$INSU::STARCVMFS."/adev";
...
# ------------------------------------------
$INSU::RELFLNMAFS=".log/afs.release";
$INSU::RELFLNMCVMFS="CVMFSRELEASE";
...
{
    my($lib,$useCVMFS)=@_;
    # 0 = AFS, 1 = CVMFS, default to AFS
    $useCVMFS = defined $useCVMFS ? $useCVMFS : (1==0);
    my($retv);

    $retv = $useCVMFS ? $INSU::COMPDIRCVMFS : $INSU::COMPDIRAFS;

    if( defined($lib) ){
	if ($lib !~ /adev/){
	    $tryv = ($useCVMFS ? "$INSU::STARCVMFS" : "$INSU::STARAFS") . "/lib";
	    if( -d $tryv){
		$retv = $tryv;
	    }
	}
...
sub IUReleaseFile
{
    ...
    # 0 = AFS, 1 = CVMFS, default to AFS
    $useCVMFS = defined $useCVMFS ? $useCVMFS : (1==0);
    return ($useCVMFS ? $INSU::RELFLNMCVMFS : $INSU::RELFLNMAFS);
}
