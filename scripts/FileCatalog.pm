...
sub _GetONames
{
    ...
    # Ex: st_physics_adc_20076059_raw_6000004.picoDst.root

    $nm =~ s/\..*//;    $name0 = $nm;  # st_physics_adc_20076059_raw_6000004
    $nm =~ s/_\d+$//;   $name1 = $nm;  # st_physics_adc_20076059_raw
    $nm =~ s/_\d+.*$//;                # st_physics_adc
    if ($nm eq $name1){
	# remove one more _.*
	if ( $name1  =~ m/(.*)(_)(.*)/){
	     $nm = $1;                 # not sure when it would be equal
	 }
    }
    $name2 = $nm;                      # likely st_physics_adc
