...
void ViewIsochrons::SetAspectRatioSwitch(const double ar) {
  if (ar < 0.) throw Exception("Value must be > 0");
  m_aspectRatio = ar;
}

void ViewIsochrons::SetLoopThreshold(const double thr) {
  if (thr < 0. || thr > 1.) throw Exception("Value must be between 0 and 1");
  m_loopThreshold = thr;
}
...
void ViewIsochrons::PlotIsochrons(
  ...
  if (tstep <= 0.) throw Exception("Time step must be > 0");
  if (points.empty()) throw Exception("No starting points provided");
  ...
  if (nDriftLines < 2) throw Exception("Too few drift lines");
  ...
  if (nContours == 0) throw Exception("No contour lines");
