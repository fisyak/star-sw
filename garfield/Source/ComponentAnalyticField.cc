...
bool ComponentAnalyticField::GetVoltageRange(double& pmin, double& pmax) {
  // Make sure the cell is prepared.
  if (!m_cellset && !Prepare()) throw Exception("Cell not set up");
  pmin = m_vmin;
  pmax = m_vmax;
  return true;
}
...
void ComponentAnalyticField::AddWire(const double x, const double y,
  ...
  if (diameter <= 0.) throw Exception("Unphysical wire diameter");
  if (tension <= 0.) throw Exception("Unphysical wire tension");
  if (rho <= 0.) throw Exception("Unphysical wire density");
  if (length <= 0.) throw Exception("Unphysical wire length");
  if (ntrap <= 0) throw Exception("Nbr. of trap radii must be > 0");
  if (m_polar && x <= diameter)
    throw Exception("Wire is too close to the origin");
  // Create a new wire.
  Wire wire;
  if (m_polar) {
    double r = 0., phi = 0.;
...
void ComponentAnalyticField::AddTube(const double radius, const double voltage,
                                     ...
                                     const std::string& label) {
  // Check if the provided parameters make sense.
  if (radius <= 0.0) throw Exception("Unphysical tube dimension");
  if (nEdges < 3 && nEdges != 0)
    throw Exception(std::string("Unphysical number of tube edges (" +
...
void ComponentAnalyticField::AddPlaneX(const double x, const double v,
                                       const std::string& label) {
  if (m_polar) throw Exception("Not compatible with polar coordinates");
  if (m_ynplan[0] && m_ynplan[1])
    throw Exception("Cannot have more than two planes at constant x");
...
void ComponentAnalyticField::AddPlaneY(const double y, const double v,
  ...
  if (m_polar) throw Exception("Not compatible with polar coordinates");
  if (m_ynplan[2] && m_ynplan[3])
    throw Exception("Cannot have more than two planes at constant y");
  if (m_ynplan[2]) {
    m_ynplan[3] = true;
    m_coplan[3] = y;
    m_vtplan[3] = v;
...
void ComponentAnalyticField::AddPlaneR(const double r, const double v,
                                       const std::string& label) {
  if (!m_polar) throw Exception("Not compatible with Cartesian coordinates");
  if (r <= 0.) throw Exception("Radius must be larger than zero");
  if (m_ynplan[0] && m_ynplan[1])
    throw Exception("Cannot have more than two circular planes");
  if (m_ynplan[0]) {
    m_ynplan[1] = true;
    m_coplan[1] = log(r);
    m_vtplan[1] = v;
...
void ComponentAnalyticField::AddPlanePhi(const double phi, const double v,
  ...
  if (!m_polar) throw Exception("Not compatible with Cartesian coordinates");
  if (m_ynplan[2] && m_ynplan[3])
    throw Exception("Cannot have more than two planes at constant phi");
  if (m_ynplan[2]) {
    m_ynplan[3] = true;
...
void ComponentAnalyticField::AddStripOnPlaneX(const char dir, const double x,
  ...
  if (m_polar || (!m_ynplan[0] && !m_ynplan[1]))
    throw Exception("There are no planes at constant x");
  if (dir != 'y' && dir != 'Y' && dir != 'z' && dir != 'Z')
    throw Exception(
        (std::string("Invalid direction (") + dir +
         std::string("). Only strips in y or z direction are possible"))
            .c_str());
  if (fabs(smax - smin) < Small)
    throw Exception("Strip width must be greater than zero");
  if (label.empty()) throw Exception("Label must not be an empty string");
  Strip newStrip;
  newStrip.type = label;
  newStrip.ind = -1;
...
void ComponentAnalyticField::AddStripOnPlaneY(const char dir, const double y,
  ...
  if (m_polar || (!m_ynplan[2] && !m_ynplan[3]))
    throw Exception("There are no planes at constant y");
  if (dir != 'x' && dir != 'X' && dir != 'z' && dir != 'Z')
    throw Exception(
        (std::string("Invalid direction (") + dir +
         std::string("). Only strips in x or z direction are possible"))
            .c_str());
  if (fabs(smax - smin) < Small)
    throw Exception("Strip width must be greater than zero");
  if (label.empty()) throw Exception("Label must not be an empty string");
...
void ComponentAnalyticField::AddStripOnPlaneR(const char dir, const double r,
                                              ...
                                              const double gap) {
  if (!m_polar || (!m_ynplan[0] && !m_ynplan[1]))
    throw Exception("There are no planes at constant r");
  if (dir != 'p' && dir != 'P' && dir != 'z' && dir != 'Z')
    throw Exception(
        (std::string("Invalid direction (") + dir +
         std::string("). Only strips in p(hi) or z direction are possible"))
            .c_str());
  if (fabs(smax - smin) < Small)
    throw Exception("Strip width must be greater than zero");
  if (label.empty()) throw Exception("Label must not be an empty string");
...
void ComponentAnalyticField::AddStripOnPlanePhi(
  ...
  if (!m_polar || (!m_ynplan[2] && !m_ynplan[3]))
    throw Exception("There are no planes at constant phi");
  if (dir != 'r' && dir != 'R' && dir != 'z' && dir != 'Z')
    throw Exception(
        (std::string("Invalid direction (") + dir +
         std::string("). Only strips in r or z direction are possible"))
            .c_str());
  if (fabs(smax - smin) < Small)
    throw Exception("Strip width must be greater than zero");
  if (label.empty()) throw Exception("Label must not be an empty string");
  ...
  if (dir == 'z' || dir == 'Z') {
    if (smin < Small || smax < Small)
      throw Exception("Radius must be greater than zero");
...
void ComponentAnalyticField::AddPixelOnPlaneX(
  ...
  if (fabs(ymax - ymin) < Small || fabs(zmax - zmin) < Small)
    throw Exception("Pixel width must be greater than zero");
  if (label.empty()) throw Exception("Label must not be an empty string");
  Pixel pixel;
  pixel.type = label;
  pixel.ind = -1;
...
void ComponentAnalyticField::SetPeriodicityX(const double s) {
  if (m_polar)
    throw Exception("Cannot use x-periodicity with polar coordinates");
  if (s < Small) throw Exception("Periodic length must be greater than zero");
  m_symmetries.Set(Symmetry::Type::PeriodicX);
  m_sx = s;
  UpdatePeriodicity();
}
...
void ComponentAnalyticField::SetPeriodicityY(const double s) {
  if (m_polar)
    throw Exception("Cannot use y-periodicity with polar coordinates");
  if (s < Small) throw Exception("Periodic length must be greater than zero");
  m_symmetries.Set(Symmetry::Type::PeriodicY);
  m_sy = s;
  UpdatePeriodicity();
}
...
void ComponentAnalyticField::SetPeriodicityPhi(const double s) {
  if (!m_polar && !m_tube)
    throw Exception("Cannot use phi-periodicity with Cartesian coordinates");
  if (std::abs(360. - s * int(360. / s)) > 1.e-4)
    throw Exception("Phi periods must divide 360");
  m_symmetries.Set(Symmetry::Type::PeriodicY);  // TODO PUT GOOD NAME FOR
                                                // CYLINDRICAL/SPERICAL
  m_sy = DegreeToRad * s;
  m_mtube = int(360. / s);
  UpdatePeriodicity();
}
...
bool ComponentAnalyticField::GetPeriodicityX(double& s) {
  if (!m_symmetries.Has(Symmetry::Type::PeriodicX) || m_polar) {
    s = 0.;
    return false;
  }
...
bool ComponentAnalyticField::GetPeriodicityY(double& s) {
  if (!m_symmetries.Has(Symmetry::Type::PeriodicY) || m_polar) {
    s = 0.;
    return false;
  }
...
bool ComponentAnalyticField::GetPeriodicityPhi(double& s) {
  if (!m_symmetries.Has(Symmetry::Type::PeriodicY) || (!m_polar && !m_tube)) {
    s = 0.;
    return false;
  }
...
void ComponentAnalyticField::UpdatePeriodicity() {
  ...
  if (m_perx && !m_symmetries.Has(Symmetry::Type::PeriodicX)) {
    m_perx = false;
    m_cellset = false;
    m_sigset = false;
  } else if (!m_perx && m_symmetries.Has(Symmetry::Type::PeriodicX)) {
    if (m_sx < Small) {
  ...
  if (m_pery && !m_symmetries.Has(Symmetry::Type::PeriodicY)) {
    m_pery = false;
    m_cellset = false;
    m_sigset = false;
  } else if (!m_pery && m_symmetries.Has(Symmetry::Type::PeriodicY)) {
    if (m_sy < Small) {
      std::cerr << m_className << "::UpdatePeriodicity:\n"
  ...
  if (m_symmetries.Has(Symmetry::Type::Mirror)) {
    std::cerr << m_className << "::UpdatePeriodicity:\n"
...
bool ComponentAnalyticField::GetWire(const std::size_t i, double& x, double& y,
  ...
  if (i >= m_nWires) throw Exception("Index out of range");
...
bool ComponentAnalyticField::ElectricFieldAtWire(const std::size_t iw,
  ...
  if (iw >= m_nWires) throw Exception("Index out of range");
...
void ComponentAnalyticField::SetScanningGrid(const std::size_t nX,
                                             const std::size_t nY) {
  if (nX < 2) throw Exception("Number of x-lines must be > 1");
  m_nScanX = nX;
  if (nY < 2) throw Exception("Number of y-lines must be > 1");
  m_nScanY = nY;
}
...
void ComponentAnalyticField::SetScanningArea(const double xmin,
  ...
  if (std::abs(xmax - xmin) < Small || std::abs(ymax - ymin) < Small)
    throw Exception("Zero range not permitted");
...
void ComponentAnalyticField::SetScanningAreaFirstOrder(const double scale) {
  if (scale <= 0.) throw Exception("Scaling factor must be > 0");
...
bool ComponentAnalyticField::ForcesOnWire(
  ...
  if (!m_cellset && !Prepare()) throw Exception("Cell not set up");
  if (iw >= m_nWires) throw Exception("Wire index out of range");
  if (m_polar) throw Exception("Cannot handle polar cells");
...
void ComponentAnalyticField::SetNumberOfSteps(const std::size_t n) {
  if (n == 0) throw Exception("Number of steps must be > 0");
  m_nSteps = n;
}
...
bool ComponentAnalyticField::WireDisplacement(
  ...
  if (!Setup())
    throw Exception("Charge calculation failed at central position");
  double fx = 0., fy = 0.;
  ...
  if (!ForcesOnWire(iw, xMap, yMap, fxMap, fyMap))
    throw Exception("Could not compute the electrostatic force table");
...
void ComponentAnalyticField::CellInit() {
  ...
  // Periodicities
  m_perx = m_pery = false;
  m_symmetries.Deactivate(Symmetry::Type::PeriodicX);
  m_symmetries.Deactivate(Symmetry::Type::PeriodicY);
  m_sx = m_sy = 1.;
...
bool ComponentAnalyticField::Prepare() {
  ...
  if (!CellCheck()) throw Exception("The cell does not meet the requirements");
  if (m_debug) std::cout << m_className << "::Prepare: Cell check ok.\n";

  // Determine the cell type.
  if (!CellType()) throw Exception("Type identification of the cell failed");
  ...
  if (!Setup()) throw Exception("Charge calculation failed");
  if (m_debug) {
    std::cout << "    Calculation of the charges was successful.\n";
  }

  // Assign default gaps for strips and pixels.
  if (!PrepareStrips()) throw Exception("Strip/pixel preparation failed");
...
bool ComponentAnalyticField::CellCheck() {
  ...
  if (nElements < 2)
    throw Exception("At least 2 elements are necessary. Cell rejected");
...
void ComponentAnalyticField::SetNumberOfCellCopies(const std::size_t nf) {
  if (nf > 0) {
    if ((nf & (nf - 1)) != 0)
      throw Exception("Argument must be an integral power of 2");
  }
  m_nFourier = nf;
  m_sigset = false;
}
...
bool ComponentAnalyticField::Update(const std::vector<double>& vw,
  ...
  if (!Setup()) throw Exception("Error computing the charges");
...
bool ComponentAnalyticField::MultipoleMoments(const std::size_t iw,
  ...
  if (iw >= m_nWires) throw Exception("Wire index out of range");
  if (eps <= 0.) throw Exception("Epsilon must be positive");
  if (nPoles == 0) throw Exception("Multipole order out of range");
  if (rmult <= 0.) throw Exception("Radius multiplication factor out of range");
...
bool ComponentAnalyticField::OptimiseOnTrack(
  ...
  // -----------------------------------------------------------------------
  if (nP < 1) throw Exception("Number of points < 1");
  if (groups.empty()) throw Exception("No electrode groups");
  ...
  if (!fPtr) throw Exception("Error in the field function");
  ...
  if (!fPtr) throw Exception("Error in the target function");
  ...
  if (aFit.empty()) throw Exception("Setting fitting parameters failed");
  ...
  if (!Numerics::LeastSquaresFit(fFit, aFit, eFit, xFit, yFit, wFit,
                                 ...
                                 m_debug))
    throw Exception("The new potentials do not fulfill your requirements");
  ...
  if (!Update(vw, vp))
    throw Exception("Failed to compute the charges for the final settings");
  return true;
}
...
bool ComponentAnalyticField::OptimiseOnGrid(
  ...
  if (nX * nY < 1) throw Exception("Number of points < 1");
  if (groups.empty()) throw Exception("No electrode groups");
...
bool ComponentAnalyticField::OptimiseOnWires(
  ...
  if (wires.empty()) throw Exception("Number of wires < 1");
  if (groups.empty()) throw Exception("No electrode groups");
