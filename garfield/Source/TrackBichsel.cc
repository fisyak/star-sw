...
bool TrackBichsel::Initialise() {
  ...
  infile.close();
  if (!ok) throw Exception("Error reading dielectric function");
  ...
  if (!infile) throw Exception("Could not open macom.tab");
  for (std::string line; std::getline(infile, line);) {
    ltrim(line);
    if (IsComment(line)) continue;
    std::istringstream data(line);
  ...
  infile.close();
  if (!ok) throw Exception("Error reading K/L shell data");
  ...
  if (!infile) throw Exception("Could not open emerc.tab");
  for (std::string line; std::getline(infile, line);) {
    ltrim(line);
    if (IsComment(line)) continue;
    std::istringstream data(line);
  ...
  infile.close();
  if (!ok) throw Exception("Error reading M shell data");
...
bool TrackBichsel::ComputeCrossSection() {
  if (!m_initialised) throw Exception("Not initialised");
...
bool TrackBichsel::NewTrack(const double x0, const double y0, const double z0,
  ...
  if (m_isChanged) {
    if (!ComputeCrossSection())
      throw Exception("Could not calculate cross-section table");
    m_isChanged = false;
  }
  ...
  if (!medium || !medium->IsIonisable())
    throw Exception("No ionisable medium at initial position");

  // Check if the medium is silicon.
  if (medium->GetName() != "Si") throw Exception("Medium is not silicon");
