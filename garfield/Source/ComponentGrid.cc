...
Medium* ComponentGrid::GetMedium(const double x, const double y,
  ...
  for (size_t i = 0; i < 3; ++i) {
    bool mirrored = false;
    Symmetry sym = m_symmetries.GetSymmetries(i);
    xx[i] = Reduce(xx[i], m_xMin[i], m_xMax[i], sym, mirrored);
  }
  // Get the indices.
...
bool ComponentGrid::GetBoundingBox(double& xmin, double& ymin, double& zmin,
  ...
  if (m_coordinates == Coordinates::Cylindrical) {
    ...
    xmax = +rmax;
    ymax = +rmax;
    zmin = (m_symmetries.Has(Symmetry::Type::PeriodicZ) ||
            m_symmetries.Has(Symmetry::Type::MirrorZ))
               ? -INFINITY
               : m_xMin[2];
    zmax = (m_symmetries.Has(Symmetry::Type::PeriodicZ) ||
            m_symmetries.Has(Symmetry::Type::MirrorZ))
               ? +INFINITY
               : m_xMax[2];
    return true;
...
bool ComponentGrid::GetField(
  ...
  for (size_t i = 0; i < 3; ++i) {
    Symmetry sym = m_symmetries.GetSymmetries(i);
    xx[i] = Reduce(xx[i], m_xMin[i], m_xMax[i], sym, mirrored[i]);
...
double ComponentGrid::Reduce(const double xin, const double xmin,
                             const double xmax, const Symmetry sym,
                             bool& mirrored) const {
...
bool ComponentGrid::GetData(
  ...
  for (std::size_t i = 0; i != 3; ++i) {
    bool Mirrored = false;
    Symmetry sym = m_symmetries.GetSymmetries(i);
    Xs[i] = Reduce(xi, m_xMin[i], m_xMax[i], sym, Mirrored);
    if (Xs[i] < m_xMin[i] || Xs[i] > m_xMax[i]) return false;
  }

  // Get the indices.
  const double sx = (Xs[0] - m_xMin[0]) * m_sX[0];
  const double sy = (Xs[1] - m_xMin[1]) * m_sX[1];
  const double sz = (Xs[2] - m_xMin[2]) * m_sX[2];
  const std::size_t i0 = static_cast<std::size_t>(std::floor(sx));
  const std::size_t j0 = static_cast<std::size_t>(std::floor(sy));
  const std::size_t k0 = static_cast<std::size_t>(std::floor(sz));
  const double ux = sx - i0;
