...
void ViewCell::Plot2d() {
  if (!Plot(true)) throw Exception("Error creating plot");
}

void ViewCell::Plot3d() {
  if (!Plot(false)) throw Exception("Error creating plot");
}

bool ViewCell::Plot(const bool twod) {
  if (!m_component && !m_nebem) throw Exception("Component is not defined");
  double pmin = 0., pmax = 0.;
  if (m_component) {
    if (!m_component->GetVoltageRange(pmin, pmax))
      throw Exception("Component is not ready");
  } else if (m_nebem) {
...
bool ViewCell::PlotNeBem(const bool twod) {
  if (!twod) throw Exception("3D plot not implemented yet");
  // Draw the regions.
