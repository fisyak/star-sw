...
bool MediumGas::SetComposition(const std::string& gas1, const double f1,
  ...
  if (m_nComponents == 0)
    throw Exception("Error setting the composition. No valid components");
  // Establish the name.
  m_name = "";
  double sum = 0.;
...
void MediumGas::SetAtomicNumber(const double /*z*/) {
  throw Exception(
      "Effective Z cannot be changed directly. Use SetComposition to define "
      "the gas mixture");
}

void MediumGas::SetAtomicWeight(const double /*a*/) {
  throw Exception(
      "Effective A cannot be changed directly. Use SetComposition to define "
      "the gas mixture");
}

void MediumGas::SetNumberDensity(const double /*n*/) {
  throw Exception(
      "Density cannot be changed directly. Use SetComposition to define the "
      "gas mixture");
}

void MediumGas::SetMassDensity(const double /*rho*/) {
  throw Exception(
      "Mass density cannot be changed directly. Use SetComposition to define "
      "the gas mixture");
}
...
bool MediumGas::LoadGasFile(const std::string& filename, const bool quiet) {
  ...
  if (!gasfile)
    throw Exception(std::string("Cannot open file " + filename).c_str());
  if (!quiet || m_debug) {
  ...
  if (!GetMixture(mixture, version, gasnames, percentages)) {
    gasfile.close();
    throw Exception("Cannot determine the gas composition");
  }
  ...
  while (gasfile.getline(line, 256)) {
    ...
    while (token) {
      if (strcmp(token, "Version") == 0) {
        token = strtok(NULL, " :,%");
        if (!token) throw Exception("Cannot read version number");
        version = atoi(token);
        // Check the version number.
        if (version != 10 && version != 11 && version != 12 && version != 13)
          throw Exception(
              std::string("The file has version number " +
        ...
        if (okstr.size() < 20)
          throw Exception(std::string("Unexpected size of GASOK string (" +
                                      std::to_string(okstr.size()) + ")")
                              .c_str());
        ...
        if (nE <= 0) throw Exception("Number of E fields out of range");
        token = strtok(NULL, " :,%\t");
        const int nA = atoi(token);
        // Check the number of angles.
        if (is3d && nA <= 0)
          throw Exception("Number of E-B angles out of range");
        token = strtok(NULL, " :,%\t");
        const int nB = atoi(token);
        // Check the number of B points.
        if (is3d && nB <= 0) throw Exception("Number of B fields out of range");
        efields.resize(nE);
        angles.resize(nA);
        bfields.resize(nB);
...
bool MediumGas::GetMixture(const std::vector<double>& mixture,
  ...
  for (std::size_t i = 0; i < nMagboltzGases; ++i) {
    ...
    if (gasname.empty())
      throw Exception(
          std::string("Unknown gas (gas number " + std::to_string(i + 1) + ")")
              .c_str());
    gasnames.push_back(gasname);
    percentages.push_back(mixture[i]);
  }
  if (gasnames.size() > m_nMaxGases)
    throw Exception(std::string("Gas mixture has " +
                                std::to_string(gasnames.size()) +
                                "components. Number of gases is limited to " +
                                std::to_string(m_nMaxGases))
                        .c_str());
  if (gasnames.empty())
    throw Exception("Gas mixture is not defined (zero components)");
...
bool MediumGas::MergeGasFile(const std::string& filename,
  ...
  if (m_nComponents != gasnames.size()) {
    gasfile.close();
    throw Exception("Composition of the dataset differs from the present one");
    return false;
  }
  ...
  if (!Similar(pgas, m_pressureTable, eps)) {
    gasfile.close();
    throw Exception(
        "The gas pressure of the dataset to be read differs from the current "
        "reference pressure");
  }
  if (!Similar(tgas, m_temperatureTable, eps)) {
    gasfile.close();
    throw Exception(
        "The gas temperature of the dataset to be read differs from the "
  ...
  if (!ReadHeader(gasfile, version, gasok, new3d, mixture, efields, bfields,
                  angles, excLevels, ionLevels)) {
    gasfile.close();
    throw Exception("Error re-reading header");
  }
  ...
  if (iemode + iamode + ibmode < 2) {
    gasfile.close();
    throw Exception(
        "Existing data and data in the file don't have two common axes");
  }
...
bool MediumGas::LoadMobility(const std::string& filename, const bool quiet,
  ...
  if (data.empty()) throw Exception("No valid data found");
...
void MediumGas::GetIonisationLevel(const size_t level, std::string& label,
                                   double& energy) const {
  if (level >= m_ionLevels.size()) throw Exception("Index out of range");
...
void MediumGas::GetExcitationLevel(const size_t level, std::string& label,
                                   double& energy) const {
  if (level >= m_excLevels.size()) throw Exception("Index out of range");
...
bool MediumGas::GetElectronIonisationRate(const size_t level, const size_t ie,
                                          ...
                                          double& f) const {
  if (level >= m_ionLevels.size()) throw Exception("Level index out of range");
...
bool MediumGas::GetElectronExcitationRate(const size_t level, const size_t ie,
                                          const size_t ib, const size_t ia,
                                          double& f) const {
  if (level >= m_excLevels.size()) throw Exception("Level index out of range");
...
bool MediumGas::DisablePenningTransfer(std::string gasname) {
  // Get the "standard" name of this gas.
  gasname = GetGasName(gasname);
  if (gasname.empty()) throw Exception("Unknown gas name");
  // Look for this gas in the present gas mixture.
  int iGas = -1;
  for (std::size_t i = 0; i < m_nComponents; ++i) {
    if (m_gas[i] == gasname) {
...
bool MediumGas::AdjustTownsendCoefficient() {
  ...
  if (m_eAlp.empty() || m_eAlp0.empty())
    throw Exception("Present gas table does not include Townsend coefficients");
  // Make sure there are excitation and ionisation rates.
  if (m_excLevels.empty() || m_excRates.empty())
    throw Exception("Present gas table does not include excitation rates");
  if (m_ionLevels.empty() || m_ionRates.empty())
    throw Exception("Present gas table does not include ionisation rates");
  const std::size_t nE = m_eFields.size();
  const std::size_t nB = m_bFields.size();
  const std::size_t nA = m_bAngles.size();
  if (m_debug) {
...
bool MediumGas::GetPhotoAbsorptionCrossSection(const double e, double& sigma,
                                               const std::size_t i) {
  if (i >= m_nMaxGases) throw Exception("Index out of range");
